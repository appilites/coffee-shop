...
1. **Seed café tree** — subcategories of **`cat-hot-beverages`** and **`cat-cold-beverages`** (espresso, iced coffee, smoothies, etc.). This repo also treats **`cat-coffee-bar`** and its subcategories as in-scope when those seed ids are used.
...
3. **ID allowlist** — if your DB uses the IDs from **`lib/mock-data.ts`** (or aliases such as `cat-meal-replacement`, `cat-loaded-tea-berry`, `cat-kids-berry`, …), those are merged in when present (`lib/unified-addons-category-scope.ts` → `UNIFIED_ADDONS_MOCK_CATEGORY_IDS`).
...
| `docs/UNIFIED_ADDONS_DATABASE.md` | DB-focused notes (API + resolution summary) |
